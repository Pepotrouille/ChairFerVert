...
<script lang='ts'>
    export let viewBoxX: number = 80;
    export let viewBoxY: number = 80;
    export let viewBoxOffsetX: number = 0;
    export let viewBoxOffsetY: number = 0;
    export let height: number = 50;
    export let width: number = 50;
    export let color: string = "white";
</script>

<svg fill={color} width={`${height}px`} height={`${width}px`} viewBox={`${viewBoxOffsetX} ${viewBoxOffsetY} ${viewBoxX} ${viewBoxY}`} xmlns="http://www.w3.org/2000/svg">
